...
def generate_auto_charts(df):
    ...
    date_col = find_column(df, ["date", "month", "year"]) or (date_cols[0] if date_cols else None)

    main_value_col = sales_col or profit_col or (numeric_cols[0] if numeric_cols else None)
    main_category_col = category_col or region_col or (categorical_cols[0] if categorical_cols else None)
    ...
    if date_col and main_value_col:
        ...
        if not temp.empty:
            ...
            grouped = temp.groupby("Period")[main_value_col].sum().reset_index()
...
def generate_plotly_charts(df):
    ...
    if main_category_col and main_value_col:
        ...
        charts.append(json.dumps(style_plotly(fig), cls=plotly.utils.PlotlyJSONEncoder))
